...
// Seeder handles transaction module seeding.
type Seeder struct {
	db *gorm.DB
}

// New creates a new transaction seeder.
func New(db *gorm.DB) *Seeder {
	return &Seeder{db: db}
}

// Seed runs all transaction seeders from SQL files.
func (s *Seeder) Seed() error {
	seedPath := "internal/modules/transaction/seeders"
	
	files, err := os.ReadDir(seedPath)
	if err != nil {
		logger.Log.Warn("No transaction seeders found", zap.Error(err))
		return nil
	}
	...
	logger.Log.Info("Transaction data seeded")
	return nil
}
